...
Наиболее эффективным методом перезапуска модуля (в случае появления обновлений, когда требуется перезагрузка) вместо всего, что описано в АЛЬТЕРНАТИВА НАЧАЛО - АЛЬТЕРНАТИВА КОНЕЦ, является способ его полного удаления и повтороной установки. Для этого выполните поочередно следующие команды:
...
Таким образом вы заново запустите один модуль. Если вы снова пожелаете запустить несколько модулей, то просто повторите последнюю команду, изменив Имя Контейнера.

Для нормальной работы модулей необходимо, чтобы они не перегружали вашу систему. Поэтому запускайте такое количество модулей, которое будет работать оптимально для ваших технических характеристик. Соответственно, в случае обнаружения работы системы на износ, удалйте лишние модули с помощью перечисленных ранее команд.
